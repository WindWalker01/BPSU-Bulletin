<div class=" min-h-screen">
  <div class="w-10xl max-w-[80%] mx-auto md:px-0 pt-8 sm:pt-12 md:pt-16">
    ...
    <div class="mt-8">
      <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold">University Announcements</h1>
       <p class="text-base sm:text-lg text-text-secondary mt-1">All the latest news and updates from the university.</p>
    </div>
        ...
        <form method="GET" action="" class="flex flex-col sm:flex-row gap-4 mt-8">
            ...
            <div class="relative w-full sm:w-40 flex-shrink-0">
                ...
                <option value="newest" class="bg-bg-dark text-text-gray" selected>Newest</option>
                <option value="oldest" class="bg-bg-dark text-text-gray ">Oldest</option>
                </select>
    ...
    <?php if (empty($blogs)): ?>
        <p class="text-text-secondary text-center mt-10 italic">No announcements found in this category.</p>
    <?php endif; ?>
    ...
    <div id="announcementContainer" class= " bg-bg-dark/50  rounded-2xl  mt-8 space-y-4">
        <?php foreach ($blogs as $b): ?>
            ...
            <a href="/blog?id=<?= $b['id'] ?>" class="block announcement-card border-2 rounded-2xl" data-date="<?= htmlspecialchars($b['created_at']) ?>">
...
<script>
  ...
  const announcementCards = announcementContainer.querySelectorAll('.announcement-card');
