<div class="min-h-screen">
  <div class="w-10xl max-w-[80%] mx-auto md:px-0 pt-8 sm:pt-12 md:pt-16">
    ...
    <div class="mt-8">
      <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold">University Achievements</h1>
      <p class="text-base sm:text-lg text-text-secondary mt-1">Student and faculty accomplishments.</p>
    </div>
    ...
    if (!empty($_GET["campus"])) {
        $filteredBlogs = array_filter($filteredBlogs, function ($b) {
    ...
    if (!empty($_GET["search"])) {
        ...
        $filteredBlogs = array_filter($filteredBlogs, function ($b) use (
            $search,
        ) {
    ...
    <?php if (empty($filteredBlogs)): ?>
      <p class="text-text-secondary text-center mt-10 italic">No Achievements found in this category.</p>
    <?php endif; ?>

    <div id="achievementsContainer" class="mt-8 space-y-4">
      <?php foreach ($filteredBlogs as $b): ?>
        ...
        <a href="/blog?id=<?= $b[
            "id"
        ] ?>" class="block achievements-card border-2 rounded-2xl" data-date="<?= htmlspecialchars(
    $b["created_at"],
) ?>"> 
    ...
    <div class="flex justify-start items-center mt-10 text-text-secondary gap-4">
      <form method="GET" action="" class="flex items-center gap-2 mb-10">
        ...
        <!-- Hidden filters persist through pagination -->
