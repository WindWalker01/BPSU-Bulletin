<article class="bg-overlay-dark/50 border border-card-dark rounded-xl backdrop-blur-sm flex overflow-hidden max-h-70">
  ...
  <div class="flex-shrink-0 w-60">
    <img class="rounded-l-xl object-cover h-full w-full" src="<?= $image ??
        "https://openlab.sps.cuny.edu/omvss/wp-content/themes/koji/assets/images/default-fallback-image.png" ?>" alt="">
  </div>
  
  <div class="flex-grow space-y-4 p-6 flex flex-col">
    <div class="flex justify-between items-center">
      <span class="text-xs font-medium <?= htmlspecialchars(
          $badgeColor ?? "bg-brand/20 text-brand",
      ) ?> px-2 py-1 rounded-full">
        <?= htmlspecialchars($category ?? "General") ?>
      </span>
      <p class="text-xs text-text-secondary">Published on <?= htmlspecialchars(
          $date ?? "Unknown Date",
      ) ?></p>
    </div>
    ...
    <div class="pt-4 border-t border-card-dark flex items-center justify-between text-sm">
      <div class="flex items-center gap-4">
        <button class="flex items-center gap-1.5 text-text-secondary hover:text-brand transition-colors cursor-pointer">
          <span class="material-symbols-outlined text-xl">thumb_up</span> <?= htmlspecialchars(
              $likes ?? 0,
          ) ?>
        </button>
        <button class="flex items-center gap-1.5 text-text-secondary hover:text-brand transition-colors cursor-pointer">
          <span class="material-symbols-outlined text-xl">chat_bubble</span> <?= htmlspecialchars(
              $comments ?? 0,
          ) ?>
        </button>
      </div>
