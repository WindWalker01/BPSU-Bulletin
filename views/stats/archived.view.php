<body class="bg-bg-dark min-h-screen">
    ...
    <div class="layout-container flex h-full grow flex-col">
      <div class="px-4 sm:px-6 lg:px-40 flex flex-1 justify-center py-5">
        <div class="layout-content-container flex flex-col max-w-[1200px] flex-1 w-full">
              ...
              <label class="flex flex-col min-w-40 h-12 w-full">
                <div class="flex w-full flex-1 items-stretch rounded-lg h-full">
                  ...
                  <?php if (!empty($search_term)): ?>
                    <a href="/archived?sort=<?= htmlspecialchars($sort_order) ?>" class="text-text-secondary flex border-none bg-overlay-dark/50 bg-blur-sm items-center justify-center pr-4 rounded-r-lg hover:text-text-primary transition-colors">
          ...
          <div class="pb-3">
            <div class="flex flex-col lg:flex-row lg:justify-between px-4 gap-4">
              <div class="flex gap-4 sm:gap-8 overflow-x-auto pb-2 lg:pb-0">
                ...
                <a class="flex flex-col items-center justify-center border-b-[3px] border-b-brand text-text-primary hover:text-text-primary pb-[13px] pt-4 whitespace-nowrap transition-colors" href="archived">
              ...
              <div class="flex gap-2 sm:gap-3 flex-wrap">
                <div class="relative">
                  <button id="sort-button" class="flex h-8 cursor-pointer shrink-0 items-center justify-center gap-x-2 rounded-lg bg-card-dark hover:bg-card-dark/80 transition-colors pl-4 pr-2">
                  ...
                  <div id="sort-menu" class="absolute z-10 top-full left-0 lg:right-0 lg:left-auto mt-2 w-56 rounded-lg bg-overlay-dark border border-card-dark shadow-lg overflow-hidden hidden dropdown-menu">
                    <ul class="py-2">
                        ...
                        ?>
                        <li><a href="/archived?sort=desc<?= $search_query ?>" class="block px-4 py-2 text-text-secondary hover:bg-card-dark hover:text-text-primary rounded-md text-sm mx-2">Most Recent</a></li>
                        <li><a href="/archived?sort=asc<?= $search_query ?>" class="block px-4 py-2 text-text-secondary hover:bg-card-dark hover:text-text-primary rounded-md text-sm mx-2">Oldest</a></li>
                    </ul>
                  </div>
                </div>
              </div>
          ...
          <h3 class="text-text-primary text-lg font-bold leading-tight tracking-[-0.015em] px-4 pb-2 pt-4">Archived</h3>
          <div class="px-4 py-3">
                  ...
                  <?php if (empty($archived_blogs)): ?>
                    <tr class="border-t border-card-dark">
                      <td colspan="5" class="px-4 py-4 text-center text-text-secondary text-sm font-normal leading-normal">
                        <?php if (!empty($search_term)): ?>
                            No archived posts found matching "<?= htmlspecialchars($search_term) ?>".
                        <?php else: ?>
                    ...
                    <?php foreach ($archived_blogs as $blog): ?>
                      <tr class="border-t border-card-dark hover:bg-card-dark/30 transition-colors">
                        ...
                        <td class="px-4 py-4 text-sm font-normal leading-normal">
                          <span class="inline-flex items-center justify-center rounded-lg px-3 py-1 bg-brand/20 text-brand-hover text-xs font-medium whitespace-nowrap">
                            Archived
                          </span>
                        </td>
                        ...
                        <td class="px-4 py-4 text-text-secondary text-sm font-bold leading-normal tracking-[0.015em] hidden md:table-cell">
                          ...
                          <form action="/unarchive" method="POST" style="display: inline;">
                              <input type="hidden" name="id" value="<?= $blog['id'] ?>">
                              <button type="submit" class="text-accent-green hover:text-accent-light-green cursor-pointer transition-colors bg-transparent border-none p-0 font-bold leading-normal">Unarchive</button>
                          </form> | 
                          ...
                          <button type="button" 
                                ...
                                data-redirect-to="/archived"
                                data-message="Are you sure you want to permanently delete this post? This action cannot be undone.">
                            Delete
                          </button>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      // Updated buttons and menus for the new Sort dropdown
      const buttons = [
      ...
      // Updated to handle click-outside properly
